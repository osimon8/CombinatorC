...
let compile_bexp_to_circuit (b: bexp) : circuit list = 
  ...
  let inp = if has_vars then Constant (inp_id, List.map (fun v -> v, 1) vars) else Pole inp_id in
  let g = CG.create () in 
  ...
  let rec cb b = 
    let binop b1 b2 aop = 
      ...
      let wire_input () = CG.add_edge g (C inp_id) (Ain id) in
      begin match b1, b2 with 
                            | Lit l1, Lit l2 -> [ Arithmetic (id, (Const l1, aop, Const l2, Symbol o_sig)) ]
                            | Lit l, Var v -> wire_input (); [ Arithmetic (id, (Const l, aop, Symbol v, Symbol o_sig)) ]
                            | Var v, Lit l -> wire_input (); [ Arithmetic (id, (Symbol v, aop, Const l, Symbol o_sig)) ]
                            | Var v1, Var v2 -> wire_input (); [ Arithmetic (id, (Symbol v1, aop, Symbol v2, Symbol o_sig)) ]
    ...
    begin match b with 
    | Var v -> [], C inp_id, v (* we don't need new combinators, the rec loop will wire to the input *)
    | Lit l -> let s = sig_ctr () in let id = entity_ctr () in [ Constant (id, [(s, l)]) ], C id, s (* lone lit is a constant *)
    | Neg b -> let c, o, s = cb b in 
              let id = entity_ctr () in 
              let o_sig = sig_ctr () in
              CG.add_edge g (C inp_id) (Ain id);
              [ Arithmetic (id, (Symbol s, Mul, Const (-1), Symbol o_sig)) ], Aout id, o_sig
...
let rec primitive_optimization (vars:string list) (circuit:circuit) : circuit = 
  ...
  let bind_conn cc oc i acc = 
    begin match lookup i combs with 
      ...
      | _ -> failwith "combinators inconsistent with connections"
      end in
  ...
  (* TODO: remap ids after combinators have been deleted *)
...
let remap_ids (circuit:circuit) : circuit = 
  let wire, combs, g = circuit in 
  let id_map = List.mapi (fun i c -> id_of_combinator c, i) combs in 

  let new_g = CG.create () in 

  let remap_conn c =
    let id = List.assoc (id_of_conn c) id_map in
     begin match c with 
    | Ain i -> Ain id
    | Aout i -> Aout id
    | Din i -> Din id
  ...
  let new_combs = List.mapi (fun i c -> begin match c with 
...
let compile_bexp_to_json ?optimize:(optimize=true) (b: bexp) : json list = 
  ...
  let circuits = 
    if optimize then 
      ...
      List.map remap_ids circuits_opt 
    else circuits in
